...
RUN git clone --depth=1 https://github.com/minotaur-toolkit/alive2-x86.git $HOME/alive2-x86
RUN mkdir $HOME/alive2-x86/build && cd $HOME/alive2-x86/build && \
  cmake -G Ninja -DLLVM_DIR=$HOME/llvm/build/lib/cmake/llvm -DBUILD_TV=1 -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DCMAKE_BUILD_TYPE=RelWithDebInfo $HOME/alive2-x86
RUN cd $HOME/alive2-x86/build && ninja
...
RUN mkdir $HOME/minotaur/build && cd $HOME/minotaur/build && \
  cmake $HOME/minotaur -DALIVE2_SOURCE_DIR=$HOME/alive2-x86 -DALIVE2_BUILD_DIR=$HOME/alive2-x86/build -DCMAKE_PREFIX_PATH=$HOME/llvm/build -DCMAKE_EXPORT_COMPILE_COMMANDS=1 -DCMAKE_BUILD_TYPE=RelWithDebInfo -G Ninja
